...
PROJECT_ID = os.getenv("GOOGLE_CLOUD_PROJECT") or "image-sense-sandbox-507448"
...
try:
    ...
    print(f"[VirtualTryOn Tool] Vertex AI initialized for project:{PROJECT_ID} location:{LOCATION}.")
except Exception as e:
    print(f"[VirtualTryOn Tool ERROR] Failed to initialize Vertex AI client or model endpoint: {e}")
    client = None  # Ensure client is None if init fails


def generate_person_image(prompt: str):
    return prompt
...
def perform_virtual_try_on(
    ...
    try:
        ...
        print(f"[Virtual Try-On Core] Try-On took {end - start:.2f}s.")

        return response
    except Exception as e:
        print("Failed to perform VTO. Exception: ", str(e))
        return None
